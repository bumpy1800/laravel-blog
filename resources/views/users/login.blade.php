...
@section('login_content')
        ...
        <form class="p-10 bg-white rounded flex justify-center items-center flex-col shadow-md" method="POST" action="{{ route('login') }}">
            ...
            <input type="email" name="email" class="@error('email') is-invalid @enderror mb-5 p-3 w-80 focus:border-purple-700 rounded border-2 outline-none " autocomplete="off" placeholder="Email" required>  
            @error('email')
                <div class="alert alert-danger text-red-600">{{ $message }}</div>
            @enderror
            <input type="password" name="password" class="@error('password') is-invalid @enderror mb-5 p-3 w-80 focus:border-purple-700 rounded border-2 outline-none " autocomplete="off" placeholder="Password" required>
            @error('password')
                <div class="alert alert-danger text-red-600">{{ $message }}</div>
            @enderror
