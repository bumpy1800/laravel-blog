...
<form class="bg-white" method="POST" action="{{ route('posts.update',['post' => $posts]) }}">
    @csrf
    @method('PATCH')
    <div class="min-h-screen md:px-20 pt-6">
      <div class=" bg-white rounded-md px-6 py-10 max-w-2xl mx-auto">
        <h1 class="text-center text-2xl font-bold text-gray-500 mb-10">글 수정</h1>
        <div class="space-y-4">
          <div>
          ...
          <div>
            ...
            <textarea class="@error('editor') is-invalid @enderror form-control" id="editor" name="editor">{{ $posts->content }}</textarea>
            @error('editor')
              <div class=" mr-16 alert alert-danger text-red-600">{{ $message }}</div>
            @enderror
          </div>
          <button type="submit" class="px-6 py-2 mx-auto block rounded-md text-lg font-semibold text-indigo-100 bg-indigo-600  ">수정 완료</button>
        </div>
      </div>
    </div>
  </form>
